...
- [ ] Production uses [docker/docker-compose.prod.yml](../docker/docker-compose.prod.yml) with **`.env.prod`** (never committed): `chmod 600`.  
...
- [ ] **Iyzico**, **FCM**, **SMS**, **SMTP**: placeholders in [.env.prod.example](../.env.prod.example); real values only on server.  
- [ ] **Rotation procedure:** document who rotates what and how clients are forced to re-auth if needed.

## Swagger and API surface

- [ ] **`SWAGGER_ENABLED=false`** in production `.env.prod`.  
- [ ] If Swagger must exist temporarily, restrict by **IP** or **VPN** at Nginx, not public.

## Backups

- [ ] Automate [scripts/backup-prod.sh](../scripts/backup-prod.sh) (cron + retention).  
...
curl -sfS https://api.example.com/api/v1/health   # replace with your API URL
```

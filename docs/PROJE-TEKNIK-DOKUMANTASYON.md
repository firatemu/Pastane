...
**Politika:** Yerel = dev; production deploy = GitHub Actions registry build/push + VPS pull-only (`pnpm push:vps`).
...
`deploy.sh` sırası: supabase-prod sağlık → registry pull + `up --no-build` → `prisma migrate deploy` → health + smoke.

...
import { messageForCode } from './audience-overrides';

const PHRASE_ENTRIES: ReadonlyArray<[RegExp, string]> = [
...
export function validationDetailsUserHint(details: unknown): string | null {
  ...
  for (const item of details) {
    if (typeof item === 'string') {
      ...
      if (!looksLikeGenericEnglish(t)) out.push(t);
      continue;
    }
      ...
      if (c) {
        for (const msg of Object.values(c)) {
          if (typeof msg === 'string') {
            ...
            if (/property\s+\S+\s+should\s+not\s+exist/i.test(t)) {
              out.push(
                'Sayfayı yenileyip tekrar deneyin; sorun sürerse uygulama veya sunucu güncel olmayabilir.',
              );
              continue;
            }
            if (!looksLikeGenericEnglish(t)) out.push(t);
...
function statusFallback(status: number, audience: ApiAudience, contextualFallback?: string): string {
  const base = contextualFallback ?? DEFAULT_ERROR[audience];
  switch (status) {
    case 400:
 ...
 */
export function mapApiErrorToTurkish(
  ...
  if (code === 'VALIDATION_FAILED') {
    ...
    if (rawEarly && !/^validation failed$/i.test(rawEarly)) {
      ...
      if (!looksLikeGenericEnglish(rawEarly)) return rawEarly;
    }
    return messageForCode('VALIDATION_FAILED', audience) ?? statusFallback(status, audience, contextualFallback);
  }

  if (code?.startsWith('HTTP_')) {
    const fromCore = messageForCode(code, audience);
    if (fromCore) return fromCore;
    const n = Number(code.replace(/^HTTP_/, ''));
    if (!Number.isNaN(n)) return statusFallback(n, audience, contextualFallback);
  }

  if (code) {
    const mapped = messageForCode(code, audience);
  ...
  if (raw) {
    if (/^\s*request failed\s*$/i.test(raw)) {
      return contextualFallback ?? DEFAULT_ERROR[audience];
    }
    const byPhrase = phraseMatch(raw);
    if (byPhrase) return byPhrase;
    if (looksLikeGenericEnglish(raw)) return statusFallback(status, audience, contextualFallback);
    return raw;
  }
 ...
 */
export function mapUnknownErrorToTurkish(
  audience: ApiAudience,
  error: unknown,
  contextualFallback?: string,
): string {
  const fb = contextualFallback ?? DEFAULT_ERROR[audience];
  ...
  if (audience === 'customer') {
    if (/^\s*api fetch failed/i.test(m)) return 'Bağlantı kurulamadı. Lütfen tekrar deneyin.';
    if (/\bapi request failed\b/i.test(m)) return fb;
    if (/products request failed/i.test(m)) return 'Ürünler yüklenemedi. Lütfen tekrar deneyin.';
    if (/reviews request failed/i.test(m)) return 'Yorumlar yüklenemedi. Lütfen tekrar deneyin.';
    if (/stores request failed/i.test(m)) return 'Mağazalar yüklenemedi. Lütfen tekrar deneyin.';
    if (/delivery zones request failed/i.test(m)) return 'Teslimat bölgeleri yüklenemedi. Lütfen tekrar deneyin.';
  }
  if (audience === 'admin' && /\bapi request failed\b/i.test(m)) return fb;
  if (looksLikeGenericEnglish(m)) return fb;
  return m;
}

export function mapPayloadToTurkish(
  audience: ApiAudience,
  status: number,
  payload: { message?: string; error?: ApiErrorShape } | null | undefined,
  contextualFallback: string,
): string {
  if (!payload) return statusFallback(status, audience, contextualFallback);
  if (payload.error) {

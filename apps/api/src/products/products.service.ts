...
import { isTimeWindowValid } from '../common/utils/time-window.util';
...
export class ProductsService {
  ...
  async list(query: QueryProductDto) {
    ...
    const where: Prisma.ProductWhereInput = {
      ...
      ...(query.minPrice !== undefined || query.maxPrice !== undefined ? { price: { ...(query.minPrice !== undefined ? { gte: new Prisma.Decimal(query.minPrice) } : {}), ...(query.maxPrice !== undefined ? { lte: new Prisma.Decimal(query.maxPrice) } : {}) } } : {}),
    };
  ...
  async adminList(query: QueryProductDto) {
    ...
    const where: Prisma.ProductWhereInput = {
      ...
      ...(query.categoryId ? { categoryId: query.categoryId } : {}),
      ...(query.search ? { OR: [{ name: { contains: query.search, mode: 'insensitive' } }, { slug: { contains: query.search, mode: 'insensitive' } }, { description: { contains: query.search, mode: 'insensitive' } }] } : {}),
    };
    const [items, total] = await this.prisma.$transaction([
      this.prisma.product.findMany({
        where,
  ...
  async get(id: string, publicOnly = false) {
    const x = await this.prisma.product.findFirst({
      where: {
        id,
        deletedAt: null,
        ...(publicOnly ? { status: { not: ProductStatus.INACTIVE }, category: { deletedAt: null, isActive: true } } : {}),
      },
      include: this.include(),
    });
  ...
  async getBySlug(slug: string) {
    const x = await this.prisma.product.findFirst({
      where: { slug, deletedAt: null, status: { not: ProductStatus.INACTIVE }, category: { deletedAt: null, isActive: true } },
      include: this.include(),
    });
    if (!x) throw new AppException(ERROR_CODES.PRODUCT_NOT_FOUND, 'Product not found', HttpStatus.NOT_FOUND);
    return withProductPresentation(x);
  ...
  async create(dto: CreateProductDto, actor?: AuthUser) {
    ...
    const created = await this.prisma.product.create({
      data: {
        ...
        saleWindowStart: dto.saleWindowStart ?? null,
        saleWindowEnd: dto.saleWindowEnd ?? null,
        slug,
  ...
  async update(id: string, dto: UpdateProductDto, actor?: AuthUser) {
    ...
    this.assertSaleWindow(nextStart ?? undefined, nextEnd ?? undefined);
  ...
  async addGroup(productId: string, dto: CreateOptionGroupDto) {
    await this.get(productId);
    return this.prisma.productOptionGroup.create({ data: { productId, ...dto } });
  }

  async addOption(productId: string, groupId: string, dto: CreateOptionDto) {
    await this.get(productId);
    const group = await this.prisma.productOptionGroup.findFirst({ where: { id: groupId, productId, deletedAt: null } });
    if (!group) throw new AppException(ERROR_CODES.PRODUCT_NOT_FOUND, 'Option group not found for product', HttpStatus.NOT_FOUND);
    return this.prisma.productOption.create({ data: { optionGroupId: groupId, ...dto, priceModifier: new Prisma.Decimal(dto.priceModifier ?? 0) } });
  }

  async setAllergens(productId: string, ids: string[]) {
    await this.get(productId);
    await this.assertAllergens(ids);
    await this.prisma.$transaction([this.prisma.productAllergen.deleteMany({ where: { productId } }), ...ids.map((allergenId) => this.prisma.productAllergen.create({ data: { productId, allergenId } }))]);
  ...
  private async assertCategory(id: string) {
    const x = await this.prisma.category.findFirst({ where: { id, deletedAt: null } });
    if (!x) throw new AppException(ERROR_CODES.CATEGORY_NOT_FOUND, 'Category not found', HttpStatus.NOT_FOUND);
  }

  private async assertAllergens(ids: string[]) {
  ...
  private assertDiscount(price: number, discount?: number) {
    if (discount !== undefined && discount > price) throw new AppException(ERROR_CODES.VALIDATION_FAILED, 'Discounted price cannot exceed price', HttpStatus.BAD_REQUEST);
  }

  private assertSaleWindow(start?: string, end?: string) {
    if (!start && !end) return;
    if (!start || !end || !isTimeWindowValid(start, end)) {
      throw new AppException(ERROR_CODES.VALIDATION_FAILED, 'Invalid Europe/Istanbul sale window', HttpStatus.BAD_REQUEST);
    }
  }

  private productData(dto: Partial<CreateProductDto>) {
    const rest = { ...dto };
    delete rest.allergenIds;
    return {
      ...rest,
  ...
  private async uniqueSlug(name: string, excludeId?: string) {
    ...
    let slug = base;
    let i = 2;
    while (await this.prisma.product.findFirst({ where: { slug, ...(excludeId ? { id: { not: excludeId } } : {}) } })) {
      slug = `${base}-${i++}`;
    }
    return slug;
  }
...
export { computeProductAvailability };

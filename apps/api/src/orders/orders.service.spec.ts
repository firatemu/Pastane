import { DeliveryStatus, DeliveryType, OrderStatus } from '@prisma/client';
...
describe('OrdersService admin operations', () => {
  it('keeps customer order detail owner-scoped without viewAll permission', async () => {
    ...
    const service = new OrdersService(prisma, {} as never, new OrderStatusService(), {} as never, { log: jest.fn() } as never, {} as never, { createOrderStatusNotification: jest.fn() } as never);
  ...
  it('reassigns courier when assigned order delivery is still ASSIGNED', async () => {
    ...
    const service = new OrdersService(prisma, {} as never, new OrderStatusService(), {} as never, audit as never, {} as never, notifications as never);
  ...
  it('adds backend-owned delivery fee for home delivery orders', async () => {
    const tx = {
      cart: { findUnique: jest.fn().mockResolvedValue({ id: 'cart-1', items: [{ productId: 'product-1', quantity: 1, customNote: null, product: { name: 'Pasta', price: '100.00', discountedPrice: null }, options: [] }] }) },
      ...
      order: { create: jest.fn().mockImplementation(({ data }) => Promise.resolve({ id: 'order-1', ...data })), findUniqueOrThrow: jest.fn().mockResolvedValue({ id: 'order-1' }) },
      orderItem: { create: jest.fn().mockResolvedValue({ id: 'item-1' }) },
      orderStatusHistory: { create: jest.fn() },
      cartItem: { deleteMany: jest.fn() },
    };
    const prisma = { $transaction: jest.fn((fn) => fn(tx)) } as never;
    const reservations = { reserve: jest.fn() } as never;
    const queues = { scheduleStockTimeout: jest.fn() } as never;
    const service = new OrdersService(prisma, reservations, new OrderStatusService(), queues, { log: jest.fn() } as never, {} as never, { createOrderStatusNotification: jest.fn() } as never);

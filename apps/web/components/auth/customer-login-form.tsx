  ...
  return (
    <form className="mt-8 space-y-5" noValidate onSubmit={submit}>
      <label className="block space-y-2 text-sm font-medium">
        ...
        <input className="w-full rounded-2xl border border-outline-soft/60 bg-surface-lowest px-4 py-3.5 text-primary outline-none transition placeholder:text-muted/60 focus:border-primary" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="905XXXXXXXXX" inputMode="numeric" autoComplete="tel" required />
      </label>
      <label className="block space-y-2 text-sm font-medium">
        <span className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Şifre</span>
        <input className="w-full rounded-2xl border border-outline-soft/60 bg-surface-lowest px-4 py-3.5 text-primary outline-none transition focus:border-primary" value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" required />
      </label>

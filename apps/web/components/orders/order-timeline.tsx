import { CUSTOMER_TIMELINE, statusLabel, timelineStatus } from '../../lib/orders/status';
import type { Order } from '../../lib/orders/types';

export function OrderTimeline({ status, history = [] }: Readonly<{ status: string; history?: Order['statusHistory'] }>): React.JSX.Element {
  const current = CUSTOMER_TIMELINE.indexOf(timelineStatus(status) as never);
  const cancelled = status === 'CANCELLED';
  const latestByStatus = new Map((history ?? []).map((item) => [timelineStatus(item.status), item]));

...
export function CartLineItem({ item, onChanged }: Readonly<{ item: CartItem; onChanged: () => Promise<void> }>): React.JSX.Element {
  ...
  return <article className="stitch-panel grid gap-3 rounded-3xl p-3 sm:grid-cols-[104px_1fr] sm:gap-4">
    ...
    <div>
      <div className="flex items-start justify-between gap-4"><div><h3 className="font-display text-xl font-semibold text-primary">{item.product.name}</h3><p className="mt-1 text-sm text-muted">{item.options.length ? item.options.map(({ option }) => `${option.name}${Number(option.priceModifier) ? ` (+${formatTry(option.priceModifier)})` : ''}`).join(', ') : 'Standart'}</p>{item.customNote ? <p className="mt-1 text-sm text-muted">Not: {item.customNote}</p> : null}</div><button className="text-sm font-semibold text-error" disabled={busy} onClick={() => void mutate('DELETE')} type="button">Sil</button></div>
